...
## (Demo only) Create balanced subsets for a fast vignette build

> **Note.** The vignette runs on a small, balanced subset of Baron/Muraro (30 cells per cell type) so it builds quickly on the Bioconductor build farm. **Every downstream result in this vignette — the optimal rank, NMI, projection, and UMAP — is therefore computed on the subset, not on the full datasets.** For a realistic analysis on your own data, drop or relax this subsetting step; results, the auto-selected rank, and the UMAP topology will differ accordingly.
...
Integrate the CellMentor results with `SingleCellExperiment` for visualization. The UMAP below is computed from the CellMentor embedding **of the subsetted query cells (30 per cell type)** — not from the full Muraro dataset. With the full data, the same code produces a denser embedding and a more structured UMAP.
